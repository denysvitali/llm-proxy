...
func TestDefaultBaseURL(t *testing.T) {
	if defaultBaseURL != "https://inference-api.nousresearch.com/v1" {
		t.Errorf("defaultBaseURL = %q, want %q", defaultBaseURL, "https://inference-api.nousresearch.com/v1")
	}
...
func TestSendRoutingHeadersAndBody(t *testing.T) {
	const rawBody = `{"model":"nous-capybara","messages":[]}`
	tests := []struct {
		name       string
		streaming  bool
		wantAccept string
...
func TestSendUnsupportedKind(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		reached = true
		t.Errorf("HTTP handler reached for kind %q; Nous must reject it without an upstream call", r.URL.Path)
	}))
	defer srv.Close()

	c := New(srv.URL, "test-token")
	for _, kind := range []backend.Kind{backend.KindAnthropic, backend.KindOpenAIResponses, ""} {
		resp, err := c.Send(context.Background(), &backend.Request{
			Kind:    kind,
			RawBody: []byte(`{}`),
		})
