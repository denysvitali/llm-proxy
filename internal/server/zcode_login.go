...
func (s *Server) zcodeLoginPage(w http.ResponseWriter, _ *http.Request) {
	...
	w.Header().Set("Content-Security-Policy", "default-src 'self'; script-src 'self' https://o.alicdn.com 'unsafe-inline' 'unsafe-eval'; connect-src 'self' https://*.aliyuncs.com https://*.alicdn.com; frame-src https://*.aliyuncs.com https://*.alicdn.com; img-src 'self' data: https://*.alicdn.com https://*.aliyuncs.com; style-src 'self' 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'; worker-src blob:")
...
(function(){
  ...
  try{
    window.initAliyunCaptcha({
      ...
      getInstance:function(instance){captcha=instance;initialized=true;setStatus('Browser verification is ready.');},
      success:function(param){
        ...
        fetch('/login/zcode/captcha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({verify_param:param})})
          ...
          .then(function(result){if(!result.ok)throw new Error(result.body.error||'proxy rejected the verification');setStatus('Browser verification saved. It is valid for about 40 seconds.','ok');button.disabled=false;})
          .catch(function(error){setStatus('Could not save verification: '+error.message,'err');button.disabled=false;});
      },
      fail:function(){setStatus('Browser verification failed. Retry when ready.','err');button.disabled=false;},
      onError:function(){setStatus('Browser verification failed. Retry when ready.','err');button.disabled=false;}
    });
  }catch(error){setStatus('Could not initialize verification: '+error.message,'err');}
  button.addEventListener('click',function(){
    if(!initialized||!captcha){setStatus('Browser verification is still loading. Retry in a moment.','err');return;}
    button.disabled=true;setStatus('Starting browser verification…');
