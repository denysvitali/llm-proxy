	...
	for {
		...
		case err := <-result:
			if err != nil {
				...
				_, _ = fmt.Fprint(w, `</div><p class="ok">Sign-in successful. Your ZCode Start Plan is ready.</p><p><a href="/">Open dashboard</a></p></section></main></body></html>`)
			}
			if canFlush {
				flusher.Flush()
			}
